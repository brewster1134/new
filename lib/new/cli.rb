...
CliMiami.set_preset :key, :style => :bold
CliMiami.set_preset :value, :color => :blue, :style => :bright
CliMiami.set_preset :task_source, :color => :white, :style => [:bold, :underline]
CliMiami.set_preset :task_path, :color => :blue, :style => :underline
CliMiami.set_preset :task_value, :color => :blue, :style => :bright
...
class New::Cli < Thor
  ...
  def init
    ...
    S.ay 'Name: ', :preset => :key, :newline => false
    S.ay name, :value
    S.ay 'Version: ', :preset => :key, :newline => false
    S.ay version.to_s, :value
    S.ay 'Tasks: ', :key
    tasks_list.each do |task|
      S.ay "#{task.source.name}##{task.name}", :preset => :value, :indent => 2
    end

    tasks_list.each do |task|
      S.ay 'Set options for: ', :newline => false
      S.ay task.name.to_s, :task_value
      S.ay

      task.options.each do |option_name, option_settings|
        S.ay option_settings[:description]
        option_value = nil
        ...
        when option_settings[:type] == Array
          ...
          option_value = nil
          until option_value
            S.ay option_name.to_s, :preset => :task_value, :newline => false
        ...
        when option_settings[:type] == Hash
          ...
          option_keys.each do |key|
            ...
            until option_value[key]
              if klass == Array
                S.ay "#{option_name}: #{key}", :preset => :task_value, :newline => false
                option_value[key] = get_array_from_user(option_name)
              else
                A.sk "#{option_name}: #{key}", :preset => :task_value do |response|
        ...
        # collect non array/hash option type value
          ...
          option_value = nil
          until option_value
            A.sk option_name.to_s, :preset => :task_value do |response|
              option_value = task.validate_option(option_name, response) rescue nil
            end
          end
        end

        newfile_object[:tasks][option_name] = option_value
      end
    end
  ...
  def tasks args = {}
    ...
    New::Source.sources.each do |source_name, source|
      ...
      source.tasks.each do |task_name, task|
        if args[:show_source]
          S.ay "#{source_name}#", :preset => :task_value, :indent => 1, :newline => false
          S.ay task_name.to_s, :preset => :key, :padding => padding, :justify => :ljust, :newline => false
        else
          S.ay task_name.to_s, :preset => :task_value, :padding => padding, :justify => :ljust, :newline => false, :indent => 3
        end
  ...
  no_commands do
    def get_array_from_user klass
      ...
      until user_response == ''
        A.sk user_array.join(', '), :preset => :task_value do |response|
